function carregar_dados()
{
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		...
		{
			if(!json.Paciente.Pacientes[i].hasOwnProperty("Tela172"))
			{
...
{
	json.Paciente.Pacientes[i].Tela172 = {};
	json.Paciente.Pacientes[i].Tela172.pontuacao = "";
	json.Paciente.Pacientes[i].Tela172.comeco = "";
	json.Paciente.Pacientes[i].Tela172.faltar = "";
	json.Paciente.Pacientes[i].Tela172.sobrar = "";
...
function carregar_paciente(json, i) {
	Object.keys(json.Paciente.Pacientes[i].Tela172).forEach(function(nome) {		
		if(json.Paciente.Pacientes[i].Tela172[nome] != "")
			document.getElementById(json.Paciente.Pacientes[i].Tela172[nome]).checked = true;
	});
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		if(json.Paciente.Pacientes[i].Nome == sessionStorage.getItem("PacienteSelecionado"))
		{
			...
			for(j = 0; j < allChecks.length; j++)
			{
				...
				{
					json.Paciente.Pacientes[i].Tela172[allChecks[j].name] = allChecks[j].getAttribute('id');
				}
			}
		}
	}
...
function voltar()
{
		...
		window.location.href = "Tela17.html"
}
...
function proximo(form) {
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		if(json.Paciente.Pacientes[i].Nome == sessionStorage.getItem("PacienteSelecionado"))
		{
			json.Paciente.Pacientes[i].TelaAtual = 173;
			var ls = JSON.stringify(json);
			localStorage.setItem('Pacientes', ls);
			salvar();
			form.action = "Tela173.html";
			break;
		}
	}
}
