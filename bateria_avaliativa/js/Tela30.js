function carregar_dados()
{
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		...
		{
			if(!json.Paciente.Pacientes[i].hasOwnProperty("Tela30"))
			{
...
{
	json.Paciente.Pacientes[i].Tela30 = {};
	json.Paciente.Pacientes[i].Tela30.quadrado = "";
...
function carregar_paciente(json, i) {
	Object.keys(json.Paciente.Pacientes[i].Tela30).forEach(function(nome) {		
		if(json.Paciente.Pacientes[i].Tela30[nome] != "")
			document.getElementById(json.Paciente.Pacientes[i].Tela30[nome]).checked = true;
	});
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		if(json.Paciente.Pacientes[i].Nome == sessionStorage.getItem("PacienteSelecionado"))
		{
			...
			for(j = 0; j < allChecks.length; j++)
			{
				...
				{
					json.Paciente.Pacientes[i].Tela30[allChecks[j].name] = allChecks[j].getAttribute('id');
				}
			}
		}
	}
...
function voltar()
{
		...
		window.location.href = "Tela28.html"
}
...
function proximo(form) {
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		if(json.Paciente.Pacientes[i].Nome == sessionStorage.getItem("PacienteSelecionado"))
		{
			json.Paciente.Pacientes[i].TelaAtual = 31;
			var ls = JSON.stringify(json);
			localStorage.setItem('Pacientes', ls);
			salvar();
			form.action = "Tela31.html";
			break;
		}
	}
}
