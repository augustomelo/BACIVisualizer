function carregar_dados()
{
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		...
		{
			if(!json.Paciente.Pacientes[i].hasOwnProperty("Tela18"))
			{
...
{
	json.Paciente.Pacientes[i].Tela18 = {};
	json.Paciente.Pacientes[i].Tela18.item1 = "";
	json.Paciente.Pacientes[i].Tela18.item2 = "";
	json.Paciente.Pacientes[i].Tela18.item3 = "";
	json.Paciente.Pacientes[i].Tela18.item4 = "";
	json.Paciente.Pacientes[i].Tela18.sensibilidade = "";
	json.Paciente.Pacientes[i].Tela18.controle = "";
...
function carregar_paciente(json, i) {
	Object.keys(json.Paciente.Pacientes[i].Tela18).forEach(function(nome) {		
		if(json.Paciente.Pacientes[i].Tela18[nome] != "")
			document.getElementById(json.Paciente.Pacientes[i].Tela18[nome]).checked = true;
	});
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		if(json.Paciente.Pacientes[i].Nome == sessionStorage.getItem("PacienteSelecionado"))
		{
			...
			for(j = 0; j < allChecks.length; j++)
			{
				...
				{
					json.Paciente.Pacientes[i].Tela18[allChecks[j].name] = allChecks[j].getAttribute('id');
				}
			}
		}
	}
...
function voltar()
{
		...
		window.location.href = "Tela175.html"
}
...
function proximo(form) {
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		if(json.Paciente.Pacientes[i].Nome == sessionStorage.getItem("PacienteSelecionado"))
		{
			json.Paciente.Pacientes[i].TelaAtual = 19;
			var ls = JSON.stringify(json);
			localStorage.setItem('Pacientes', ls);
			salvar();
			form.action = "Tela19.html";
			break;
		}
	}
}
