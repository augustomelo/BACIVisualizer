function carregar_dados()
{
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		...
		{
			if(!json.Paciente.Pacientes[i].hasOwnProperty("Tela27"))
			{
...
{
	json.Paciente.Pacientes[i].Tela27 = {};
	json.Paciente.Pacientes[i].Tela27.quadrado = "";
...
function carregar_paciente(json, i) {
	Object.keys(json.Paciente.Pacientes[i].Tela27).forEach(function(nome) {		
		if(json.Paciente.Pacientes[i].Tela27[nome] != "")
			document.getElementById(json.Paciente.Pacientes[i].Tela27[nome]).checked = true;
	});
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		if(json.Paciente.Pacientes[i].Nome == sessionStorage.getItem("PacienteSelecionado"))
		{
			...
			for(j = 0; j < allChecks.length; j++)
			{
				...
				{
					json.Paciente.Pacientes[i].Tela27[allChecks[j].name] = allChecks[j].getAttribute('id');
				}
			}
		}
	}
...
function voltar()
{
		...
		window.location.href = "Tela26.html"
}
...
function proximo(form) {
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		if(json.Paciente.Pacientes[i].Nome == sessionStorage.getItem("PacienteSelecionado"))
		{
			json.Paciente.Pacientes[i].TelaAtual = 28;
			var ls = JSON.stringify(json);
			localStorage.setItem('Pacientes', ls);
			salvar();
			form.action = "Tela28.html";
			break;
		}
	}
}
