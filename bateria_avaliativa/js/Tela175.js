function carregar_dados()
{
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		...
		{
			if(!json.Paciente.Pacientes[i].hasOwnProperty("Tela175"))
			{
...
{
	json.Paciente.Pacientes[i].Tela175 = {};
	json.Paciente.Pacientes[i].Tela175.pontuacao = "";
	json.Paciente.Pacientes[i].Tela175.comeco = "";
	json.Paciente.Pacientes[i].Tela175.faltar = "";
	json.Paciente.Pacientes[i].Tela175.sobrar = "";
...
function carregar_paciente(json, i) {
	Object.keys(json.Paciente.Pacientes[i].Tela175).forEach(function(nome) {		
		if(json.Paciente.Pacientes[i].Tela175[nome] != "")
			document.getElementById(json.Paciente.Pacientes[i].Tela175[nome]).checked = true;
	});
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		if(json.Paciente.Pacientes[i].Nome == sessionStorage.getItem("PacienteSelecionado"))
		{
			...
			for(j = 0; j < allChecks.length; j++)
			{
				...
				{
					json.Paciente.Pacientes[i].Tela175[allChecks[j].name] = allChecks[j].getAttribute('id');
				}
			}
		}
	}
...
function voltar()
{
		...
		window.location.href = "Tela174.html"
}
...
function proximo(form) {
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		if(json.Paciente.Pacientes[i].Nome == sessionStorage.getItem("PacienteSelecionado"))
		{
			json.Paciente.Pacientes[i].TelaAtual = 18;
			var ls = JSON.stringify(json);
			localStorage.setItem('Pacientes', ls);
			salvar();
			form.action = "Tela18.html";
			break;
		}
	}
}
